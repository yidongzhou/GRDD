#' @title Local Fréchet Regression for Euclidean data
#'
#' @description
#' Performs local Fréchet regression for Euclidean data with respect to the Euclidean metric.
#'
...
#' \dontrun{
#' # Example:
#' # result <- lfr_euc(y, x, xOut, optns = list(bw = 0.5, kernel = "gaussian"))
#' }
#'
#' @export

lfr_euc <- function(y = NULL, x = NULL, xOut = NULL, optns = list()){
  ...
  # if (is.null(optns$bw)) {
  #   optns$bw <- bwCV_euc(y = y, x = x, xOut = xOut, optns = optns)
  # } else {
...
# bwCV_euc <- function(y, x, xOut, optns) {
#   n <- nrow(x)
#   p <- ncol(x)
#   
#   # k-fold
...
#       for (j in testidx) {
#         yOutj <- lfr_euc0(x[-testidx, , drop = FALSE], x[j, ], y[-testidx, , drop = FALSE], h, optns$kernel)
