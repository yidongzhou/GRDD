#' @title Local Fréchet Regression for Network Data
#'
#' @description
#' Performs local Fréchet regression for network data represented as graph Laplacians with respect to the Frobenius metric.
#'
#' @param y A list of length \code{n}, where \code{y[[i]]} contains the graph 
#' Laplacian matrix for the \eqn{i}th network.
...
#' \dontrun{
#' # Example:
#' # result <- lfr_net(y, x, xOut, optns = list(bw = 0.5, kernel = "gaussian"))
#' }
#'
#' @export

lfr_net <- function(y = NULL, x = NULL, xOut = NULL, optns = list()){
  ...
  # if (is.null(optns$bw)) {
  #   optns$bw <- bwCV_net(y = y, x = x, xOut = xOut, optns = optns)
  # } else {
  ...
  yOut <- lapply(seq_len(nOut), function(i) {
    ...
    if (any(w < 0)) {
      ...
      yOuti <- matrix(model$Solve()$x, ncol = m)
    } else {
      yOuti <- matrix(yOuti, ncol = m)
    }
    yOuti <- (yOuti + t(yOuti)) / 2
    yOuti[yOuti > 0] <- 0
    diag(yOuti) <- 0
    diag(yOuti) <- -colSums(yOuti)
    colnames(yOuti) <- rownames(yOuti) <- nodes
    yOuti
  })
...
# bwCV_net <- function(y, x, xOut, optns) {
#   n <- nrow(x)
#   p <- ncol(x)
#   
#   # k-fold
...
#       for (j in testidx) {
#         yOutj <- lfr_net0(x[-testidx, , drop = FALSE], x[j, ], y[-testidx, , drop = FALSE], h, optns$kernel)
