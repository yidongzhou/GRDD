#' @title Local Fréchet Regression for Symmetric Positive-Definite (SPD) Matrices
#'
#' @description
#' Performs local Fréchet regression for SPD matrices with respect to the Frobenius metric.
#'
...
#' \dontrun{
#' # Example:
#' # result <- lfr_spd(y, x, xOut, optns = list(bw = 0.5, kernel = "gaussian"))
#' }
#'
#' @export

lfr_spd <- function(y = NULL, x = NULL, xOut = NULL, optns = list()){
  ...
  # if (is.null(optns$bw)) {
  #   optns$bw <- bwCV_spd(y = y, x = x, xOut = xOut, optns = optns)
  # } else {
  ...
  yOut <- lapply(seq_len(nOut), function(i) {
    ...
    if (any(w < 0)) {
      yOuti <- Matrix::nearPD(yOuti, corr = all(abs(diag(yOuti) - 1) < 1e-5), base.matrix = TRUE)$mat
    }
...
# bwCV_spd <- function(y, x, xOut, optns) {
#   n <- nrow(x)
#   p <- ncol(x)
#   
#   # k-fold
...
#       for (j in testidx) {
#         yOutj <- lfr_spd0(x[-testidx, , drop = FALSE], x[j, ], y[-testidx, , drop = FALSE], h, optns$kernel)
